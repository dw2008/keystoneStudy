...
//#include "Tensor.hpp"
//#include "Layer.hpp"
//#include "Network.hpp"//#ifdef BENCHMARK
...
//std::cout << "-----------------------------" << std::endl;
////#endif // prepare the input data
//auto input = sv::Tensor<double>(128, 128, 3); // 224 x 224 x 3
//input.randam(); // prepare the network
//auto network = sv::Network<double>(); // add layers into the network
...
//network.addLayer(layer1_3); auto *layer2_1 = new
//        sv::ConvLayer<double>(8, 3, 16); // 112 x 112 x 128
...
//network.addLayer(layer2_3); auto *layer3_1 = new
//        sv::ConvLayer<double>(16, 3, 32); // 56 x 56 x 256
...
//network.addLayer(layer3_3); auto *layer4_1 = new
//        sv::ConvLayer<double>(32, 3, 64); // 28 x 28 x 512
...
//network.addLayer(layer4_3); auto *layer5_1 = new
//        sv::ConvLayer<double>(64, 3, 64); // 14 x 14 x 512
...
//network.addLayer(layer6_3); // predict the result by forwarding
//sv::Tensor<double> output;
//network.predict(input, output);//#ifdef BENCHMARK
//std::cout << "Total:\t" << MEMALL / 1000000 << "Mb,\t"
//<< PARAMALL / 1000000 << "M,\t" << MACALL / 1000000 << "M" <<
//std::endl;
//std::cout << "-----------------------------" << std::endl;
////#endif // show the result
